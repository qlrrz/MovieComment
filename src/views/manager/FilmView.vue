...
<template>
  ...
  <div class="card" style="margin-bottom: 5px">
    <el-input v-model="data.name" placeholder="请输入电影名称" style="width: 300px; margin-right: 10px"></el-input>
    ...
    <div class="card" style="margin-bottom: 5px">
      ...
      <el-col :span="6" v-for="item in data.tableData" :key="item.id" style="margin-bottom: 20px; cursor: pointer" @click="goDetail(item.id)">
        ...
        <div style="margin: 5px 0; display: flex; align-items: center">
          ...
          <div style="flex: 1; text-align: right; color: blue">{{item.commentNum}}人评论</div>
        </div>
        <div style="font-size: 13px; color: #666;" class="line3">{{item.discription}}</div>
      </el-col>
      </el-row>
    </div>
...
//查询所有电影分类信息
const loadCategory = () => {
  request.get('/category/selectPage').then(res=> {
