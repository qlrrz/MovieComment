<template>
    <div>
        <div class="card" style="margin-bottom: 5px">
            <el-input v-model="data.name" placeholder="请输入标题查询" style="width: 300px; margin-right: 10px"></el-input>
        ...
        <el-dialog v-model="data.formVisible" title="电影信息" width="40%">
            <el-form :model="data.form" label-width="80px" style="padding-right: 20px">
              ...
              <el-form-item label="描述">
                <el-input :rows="5" type="textarea" v-model="data.form.discription" autocomplete="off" placeholder="请输入描述"/>
              </el-form-item>
...
const handleEdit = (row) => {
  data.form = JSON.parse(JSON.stringify(row));  //给表单幅值给当前的行对象,深度拷贝
  data.formVisible = true;
}
...
//删除事件
const del = (id) => {
  ElMessageBox.confirm('删除数据无法恢复，您确认吗？','确认删除', {type: 'warning'}).then(res => {
